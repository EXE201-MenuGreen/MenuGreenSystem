...
class MicroLearningRepository {
  MicroLearningRepository({ApiClient? apiClient}) : _api = apiClient ?? ApiClient();
  ...
  Future<List<MicroLearningCategory>> getCategories() async {
    ...
    return data
        .whereType<Map>()
        .map((item) => MicroLearningCategory.fromJson(item.cast<String, dynamic>()))
        .toList();
  }

  Future<void> recordAction(String id, String action) async {
    final response = await _api.postJson(ApiEndpoints.microLearningCardAction(id), {'action': action});
    if (response.statusCode != 200) throw Exception(_message(response));
  }

  Future<QuizSubmitResult> submitQuiz(String id, int selectedOptionIndex) async {
    final response = await _api.postJson(ApiEndpoints.microLearningSubmitQuiz(id), {
