...
|---------|-----|---------------------|
| Grafana | http://localhost:3000 | admin / REDACTED_ADMIN_PASSWORD |
...
curl -s -u admin:REDACTED_ADMIN_PASSWORD http://localhost:3000/api/search | jq -r '.[].uri' | while read uri; do
  name=$(basename $uri)
  curl -s -u admin:REDACTED_ADMIN_PASSWORD "http://localhost:3000/api/dashboards/$uri" | \
...
curl -X PUT -H "Content-Type: application/json" \
  ...
  http://admin:REDACTED_ADMIN_PASSWORD@localhost:3000/api/user/password
```
